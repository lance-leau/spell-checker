    ...
    if (isSpace) {
        ...
        if (!isWord(data->tree, cur)) {
            ...
            else
            {
                ...
                printf("Word: %s\nCorrected Word: %s\n\n", cur, tmp);
                // Replace the incorrect word with the corrected word in the entry
                // This step is added to update the entry text
                GtkEditable *editable = GTK_EDITABLE(entry);
                gtk_editable_delete_text(editable, n - strlen(cur), -1);
                gtk_editable_insert_text(editable, tmp, -1, &n);
...
int main(int argc, char *argv[]) {
    ...
    // destroyHashMap(map); // Uncomment this line if you have an appropriate function to free HashMap resources
